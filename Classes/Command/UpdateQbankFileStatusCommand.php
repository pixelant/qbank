...
class UpdateQbankFileStatusCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($updateQueueu as $file) {
            ...
            $remoteUpdate = $media->getUpdated()->getTimestamp();
            $message = '%s sys_file, set remote change to "%s" on sys_file with uid "%s".';
            if ($isTestOnly) {
                $prefix = 'TESTONLY: Would update';
            } else {
                $prefix = 'Update';
                $qbankService->updateFileRemoteChange($file['uid'], $media->getUpdated()->getTimestamp());
            }
            $io->writeln(sprintf($message, $prefix, $remoteUpdate, $file['uid']));
        }
